...
[CodeGraph](https://github.com/colbymchenry/codegraph) 是这个赛道里做得不错的工具——预建索引、自动同步、支持 19+ 语言、还能检测 Web 框架路由，集成了不少 AI IDE。

但它的核心思路是**索引优先**：先把代码图存进 SQLite，AI 查的是快照，不是当前文件。文件变了之后，有一个 2 秒的同步窗口，索引覆盖不到的语言或写法会被跳过。

Radar 的思路相反——**不做索引，每次调用直接分析当前文件**。没有同步延迟，没有缓存失效，结果永远和磁盘上的代码一致。

| | Radar | CodeGraph |
|---|---|---|
| 分析时机 | 调用即分析，实时 | 预索引 + 文件监听（2s 窗口） |
| 运行时依赖 | 单二进制，无 runtime | Node.js / npm |
| 冷启动 | < 50ms | 需索引预热 |
| 函数级精度 | tree-sitter AST，5 种语言 | 以文件级为主 |
| 核心定位 | 依赖图 + 调用链 + 影响范围 | 降低 AI agent token 消耗 |

两个工具的侧重点不同。如果你的首要需求是"改这个函数之前，精确知道会波及什么"，Radar 是更直接的答案。
